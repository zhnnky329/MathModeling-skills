...
> [!NOTE]
> **本次更新 — 从「自动驾驶」改回「辅助」。** 原来这套会把整场比赛从头跑到尾，你只负责点「确认」——这其实是代写，既不合赛事规则，你也学不到东西。所以评委要打分的建模判断（选哪个方法、为什么、结果好不好、数字什么含义）现在都**归你**，由两个人类门（G2.5、G4.5）把关；skill 从 24 增到 28。原来的全自动版本完整保留在 [**`legacy-full-auto`**](https://github.com/zhnnky329/MathModeling-skills/tree/legacy-full-auto) 分支——想要旧行为就切到那个分支。
